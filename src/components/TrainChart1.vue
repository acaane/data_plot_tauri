...
function parseTime(time: string): number {
    // 手动拼接Z，强制Date以UTC解析，保证转换完成的时间戳和输入的字符串时间相同，不会差8小时
    const utcTime = time.endsWith('Z') ? time : time + 'Z'
    return Date.parse(utcTime) - 8 * 60 * 60 * 1000   // Date.parse 会按 UTC 计算时间戳
}
...
async function loadData() {
    ...
    try {
        ...
        const series = Object.entries(data).map(([trainNum, records]) => {
            // 按时间排序
            // console.log(`num ${trainNum} count:${records.length}`)
